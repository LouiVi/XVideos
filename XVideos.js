//var address = "https://www.xvideos.com/video22101009/corking_hot_and_seductive_marvelous_girl";
//var address = "https://www.xvideos.com/video28278377/monster_gapes_from_cute_little_teeanger_who_need_only_fuck_her_ass";
var address = "https://www.xvideos.com/video51019383/cum_in_my_mouth_and_on_my_pussy_because_i_love_your_jizz";
//https://www.xvideos.com/video12705017/nubilefilms_intense_creampie_fuck";
//https://www.xvideos.com/video37142213/the_priviledged_dude_bangs_a_19_years_old_babe_to_feed_her_mouth";
//https://www.xvideos.com/video40389113/redheaded_teen_gets_multiple_creampies...and_wants_more_megan_winters_";

//https://www.xvideos.com/video25186741/natural_babe_creampied";
//https://www.xvideos.com/video76950691/first_day_of_vocation_with_my_eighteen_girlfriend_ivone_who_is_need_to_be_fucked";
//https://www.xvideos.com/video28425181/bangbros_-_stepdaughter_jade_jantzen_sees_her_stepdad_s_dick_bbc13674_";
//https://www.xvideos.com/video1709416/best_friends_fucking_together_amber_hardin_shirley";
//https://www.xvideos.com/video27996555/horny_step-sister_gaggs_on_step-brothers_dick_and_rough_fuck_leads_to_cumshot";
//video22311509/firstanalquest.com_-_anal_rimming_makes_her_wet_for_his_big_throbbing_cock";
//video29613067/free_upload_legal_age_teenager_porn_sex";
//video4179963/pov-teens-from-russia-scene5";
//video16793715/corking_sexy_and_seductive_pretty_girl";
//video16851901/hot_hotty_licks_big_dick";
//video4083745/anal_teen_angel_nikea";
//video2867244/fellow_undresses_his_sex_appeal_girl";
//video2868740/tight_pussy_and_anal_fuck";
...
function GetPage()
{
dlg = app.CreateDialog( "Video Pages", "TouchModal" );
	/*dlg = app.CreateListDialog( "Video Pages", "" + app.ReadFile("VideosLinksLast.txt").split("\r").join(", "));
    dlg.SetOnTouch( dlg_OnTouch );
    dlg.SetTitleColor( "#db34ca" );
    //alert(dlg.typeId);   
    dlg.Show();*/
    layDlg = app.CreateLayout( "Linear", "VCenter,FillXY" );
    layDlg.SetSize( 0.9987, 0.79893 );
    dlg.AddLayout( layDlg );
    
    vids = Change2Points(app.ReadFile("VideosLinksLast1.txt"));//.replace("\r", ",");//split("\r").join(",");
    //app.SetClipboardText( vids );
    //spinner = app.CreateSpinner(  vids, 0.98, -1);
    spinner = app.CreateList(  vids, 0.98, -1, "Alum");
    spinner.SetTextSize( 20, "ps" );
    spinner.SetTextColor( "#a92f5422" );
    spinner.SetDescription( "Videos Links List" );
    
    //spinner.SetOnChange( Change );
    spinner.SetOnTouch( Touch );
    layDlg.AddChild( spinner );
    
    
    btnDlg = app.CreateButton( "Close Dialog", 0.6, 0.1, "Lego" );
    btnDlg.SetOnTouch( function (){dlg.Dismiss();});//btnDlg_OnTouch );
    layDlg.AddChild( btnDlg );

    dlg.Show();
}

function Touch(title, body, icon, index)
{
alert(index + ": " + title + ", " + body);
	app.HttpRequest( "GET", title, null, null, handleReply );
    dlg.Dismiss();
}

function Change(item, index)
{
	app.SetClipboardText( item);
    //app.ShowPopup( item );
    //address = item;
    alert(item);
    app.HttpRequest( "GET", item, null, null, handleReply );
    dlg.Dismiss();
}


function dlg_OnTouch( title, checked )
{
...
function handleReply( error, reply )
{
    ...
    {
    		reply = reply.replace("<head>", "<head><script src='file:///android_asset/app.js'></script>"); 
        /*var funfact = reply.slice( reply.indexOf("<i>") + 3, reply.indexOf("</i>") );
        alert( funfact );*/
        //alert(reply);
        //app.SetClipboardText( reply );
        home.SetHTML(reply, "https://www.xvideos.com/");
    }
}
